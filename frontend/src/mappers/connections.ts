...
export function mapConnectionRecord(apiConn: ConnectionApiRecord): ConnectionListItem {
  ...
  return {
    ...
    credential_revision: apiConn.credential_revision ?? 1,
    credentials_updated_at: apiConn.credentials_updated_at ?? null,
    has_ssl_root_certificate: apiConn.has_ssl_root_certificate ?? false,
    has_ssl_client_certificate: apiConn.has_ssl_client_certificate ?? false,
    has_ssl_client_private_key: apiConn.has_ssl_client_private_key ?? false,
    scope_mode: apiConn.scope_mode ?? 'all',
    included_schemas: apiConn.included_schemas ?? [],
    included_tables: apiConn.included_tables ?? [],
    scope_revision: apiConn.scope_revision ?? 1,
    scope_updated_at: apiConn.scope_updated_at ?? null,
    health_check_enabled: apiConn.health_check_enabled ?? false,
    health_check_interval_minutes: apiConn.health_check_interval_minutes ?? 60,
    next_health_check_at: apiConn.next_health_check_at ?? null,
    schema_refresh_enabled: apiConn.schema_refresh_enabled ?? false,
    schema_refresh_interval_hours: apiConn.schema_refresh_interval_hours ?? 24,
    next_schema_refresh_at: apiConn.next_schema_refresh_at ?? null,
  };
}
